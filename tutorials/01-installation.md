# 101 - 01 Compile and Install Carbon

## Golang Environment

Carbon is implemented in Golang and C++.
...
```bash
# enter workspace
mkdir -p $GOPATH/src/github.com/carbonsink
cd $GOPATH/src/github.com/carbonsink

# download
git clone https://github.com/carbonsink/go-carbon.git
...
Go dependencies in Go-Carbon are managed by [Dep](https://github.com/golang/dep).
...
```
- Build NEB
You can now build the executable for Carbon:
...
Before launching a new Carbon chain, we have to define the configuration of genesis block.
...
network {
    # For the first node in a new Carbon chain, `seed` is not need.
    # Otherwise, every node need some seed nodes to introduce it into the Carbon chain.
...
A lot of examples can be found in `$GOPATH/src/github.com/carbonsink/go-carbon/conf/`

## Run Nodes

> The Carbon chain you are running at this point is private and is different with official Testnet and Mainnet.

Start your first Carbon node with the following commands.
...
By default, the node using `conf/default/config.conf` won't mine new blocks. Start your first Carbon mining node with another commands.
...
After the node starts, if the connection with the seed node is successful, you can see the following log (detailed log can be found in: `logs/miner.1/neb.log`): ![node start](../resources/pics/101-01-node-start.png)
...
[Sending Transactions on Carbon](02-transaction.md)
